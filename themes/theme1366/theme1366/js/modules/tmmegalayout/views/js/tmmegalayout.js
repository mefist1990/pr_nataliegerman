...
function mobileMenu() {
  ...
  if ($(document).width() + scrollCompensate() <= 1199 && !isMobile) {
    if (top_menu.length) {
      ...
      if (favorite.length) {
        favorite.parent().addClass('favorite');
        favorite.appendTo(mega_menu_container);
      }
      if (cart.length) {
    ...
    favorite = mega_menu_container.find('.favorite_link');
    ...
    if (favorite.length) {
      favorite.appendTo($('header .favorite'));
    }
    if (cart.length) {
